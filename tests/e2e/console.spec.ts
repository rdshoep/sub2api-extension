...
test('core console flows against mock Sub2API', async ({ page }) => {
  ...
  const accountToday = await page.getByTestId('metric-accountCost').textContent()
  const actualToday = await page.getByTestId('metric-actualCost').textContent()
  await page.getByTestId('range-24h').click()
  await expect.poll(async () => page.getByTestId('metric-accountCost').textContent()).not.toBe(accountToday)
  await expect.poll(async () => page.getByTestId('metric-actualCost').textContent()).not.toBe(actualToday)
